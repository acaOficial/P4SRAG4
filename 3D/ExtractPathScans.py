...
def save_obs_to_json(obs, filename):
    """Guarda los datos de obs en un archivo JSON, eliminando valores NaN."""
    obs_converted = {}
    for key, value in obs.items():
        filtered_data = []
        for arr in value:
            # Convertir numpy array a lista y eliminar NaN
            filtered_list = [v for v in arr if not np.isnan(v)]
            filtered_data.append(filtered_list)
        obs_converted[key] = filtered_data
    
    with open(filename, 'w') as f:
        json.dump(obs_converted, f)


def save_path_to_json(path, filename):
    """Guarda los datos de path en un archivo JSON."""
    path_converted = {}
    for key, value in path.items():
        # Convertir listas a estructuras serializables (si no lo son ya)
        path_converted[key] = list(value)
    
    with open(filename, 'w') as f:
        json.dump(path_converted, f)


if __name__ == "__main__":
    path, obs = extract_path_scans("../mydate2024_11_29_17_39_09.log", 0.0)
    if obs:
        save_obs_to_json(obs, "obs_data.json")
    if path:
        save_path_to_json(path, "path_data.json")